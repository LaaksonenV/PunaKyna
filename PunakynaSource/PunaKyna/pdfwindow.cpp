...
void PDFWindow::setDocument(QPdfDocument *document)
{
    m_pageRenderer->setDocument(document);

    m_document = document;
    connect(m_document, &QPdfDocument::statusChanged, this, &PDFWindow::documentStatusChanged);

    documentStatusChanged();
}

void PDFWindow::setZoom(qreal factor)
{
    m_zoom = factor;
...
void PDFWindow::setZoomWindowWidth()
{
    if (m_pageSizes.isEmpty() || m_pageSizes.count() < m_topPageShowing)
        return;
    QSize wind = parentWidget()->size();
    qreal zoomFactor = wind.width()/m_pageSizes.at(m_topPageShowing).width();
    // For some reason atleast on windows, the page size is bigger than should be
#ifndef Q_OS_MACOS
    zoomFactor *= 0.74;
#endif
    setZoom(zoomFactor);
    setFixedWidth(wind.width());
}

QSizeF PDFWindow::pageSize(int page)
{
...
void PDFWindow::documentStatusChanged()
{
    m_pageSizes.clear();
    m_topPageShowing = 0;
...
void PDFWindow::pageLoaded(int page, qreal zoom, QImage image)
{
    Q_UNUSED(zoom)
...
int PDFWindow::pageCount()
{
    return m_pageSizes.count();
}

void PDFWindow::paintEvent(QPaintEvent * event)
{
...
qreal PDFWindow::yForPage(int endPage)
{
