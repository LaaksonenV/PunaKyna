...
copyright mathcheck_cc( "mathcheck.cc", "Antti Valmari", 20170828 );
/*
...
const char *tkn_str[] = {
  ...
  "f_ban", "f_ban_der", "f_nodes", "f_top_opr", "floor", "forget_errors",
  ...
  "ve", "verbose_off", "verbose_on", "vp", "vt", "vv",
  "xi",
  "ze",
  "|", "|^", "|_", "||",
...
enum tkn_type {
  ...
  tkn_Uharr, tkn_UlArr, tkn_UrArr, tkn_UhArr, tkn_Uforall, tkn_Uexist,
  tkn_Uminus, tkn_Usqrt, tkn_Uand, tkn_Uor, tkn_Une, tkn_Ule, tkn_Uge,
  ...
  tkn_f_ban, tkn_f_ban_der, tkn_f_nodes, tkn_f_top_opr, tkn_floor,
  tkn_forget_err,
  ...
  tkn_ve, tkn_verbose_off, tkn_verbose_on, tkn_vp, tkn_vt, tkn_vv,
  tkn_xi,
  tkn_ze,
  ...
  // move the ifndef-part here
#endif
  tkn_err, tkn_number, tkn_decimal, tkn_eoi, tkn_fact
};
...
enum op_type {
  ...
  op_plus, op_minus, op_iprod, op_vprod, op_div, op_pow, op_root,
  op_abs, op_sqrt, op_fact,
  op_e2718, op_ln, op_log,
  op_sin, op_cos, op_tan, op_cot, op_sinh, op_cosh, op_tanh, op_ddn, op_dup,
  op_sum, op_deriv,
  op_lt, op_lq, op_eq, op_nq, op_gt, op_gq,
  op_not, op_and, op_or, op_larr, op_harr, op_rarr, op_forall, op_exists,
  op_is_int,
  op_limp, op_leq, op_impl,
...
op_type op_tkn( tkn_type tkn ){
  ...
  case tkn_Uand: case tkn_2amp: case tkn_ud: case tkn_2sup: case tkn_and:
    return op_and;
  case tkn_Uor: case tkn_du: case tkn_or: case tkn_vv: case tkn_2vbar:
    return op_or;
...
inline bool is_lrel( op_type opr ){
  return opr == op_limp || opr == op_leq || opr == op_impl;
}
...
inline bool is_srel( op_type opr ){
  return
    opr == op_lt || opr == op_lq || opr == op_eq ||
    opr == op_gt || opr == op_gq || opr == op_nq ||
    opr == op_limp || opr == op_leq || opr == op_impl;
}
...
inline bool yields_logic( op_type opr ){
  return
    ...
    opr == op_not || opr == op_and || opr == op_or ||
    opr == op_rarr || opr == op_harr || opr == op_forall || opr == op_exists;
}
...
enum var_type { vtp_R, vtp_Q, vtp_Z, vtp_mod, vtp_tv2, vtp_tv3, vtp_idx };
unsigned mod_base = 0;
...
void out_op( op_type opr, unsigned vv = 0 ){
  if( opr == op_deriv ){
    ...
    out_print( "invisible multiplication" );
  }else{
    out_am( op_AM[ opr ] );
...
inline unsigned test_values( var_type vt ){
  switch( vt ){
  case vtp_R: case vtp_Q: return test_size_R;
  case vtp_Z: return test_size_Z;
...
unsigned find_or_add_variable(
  ...
  /* Adjust the call parameter type if necessary. */
  if( vt == vtp_R && intvar( nm ) ){ vt = vtp_Z; }
  ...
  if( is_new ){
    ...
    var_used[ ii ].type != vt && !( var_used[ ii ].type == vtp_idx )
  ){
...
enum {
  ...
  ap0, ap1, ap2, ap3, ap4, ap5, ap6, ap7, ap8, ap9, ap10, ap11, ap12, ap13,
  ap14, ap15,
  gpl, gpr, gp_force
};
...
private:
  ...
  unsigned extra_;              // some bytes of workspace
...
public:
  ...
  /* Accessors */
  inline op_type opr(){ return opr_; }
  inline expression *left(){ return left_; }
  inline expression *right(){ return right_; }
  inline number val(){ return val_; }
  inline unsigned type(){ return type_; }
  inline unsigned extra(){ return extra_; }
  inline unsigned var_idx(){ return to_var_idx( val_ ); }
  inline variable &var(){ return to_var( val_ ); }
  ...
  */
  ...
  void print_AM(
    ...
    if( opr_ == op_pow ){ lprec = ap14; rprec = ap13; right_par = true; }
    else if( opr_ == op_fact ){ lprec = ap14; }
    else if( opr_ == op_sqrt ){ rprec = ap11; right_par = true; }
    else if( opr_ == op_root ){ rprec = ap11; right_par = true; }
    else if( opr_ == op_div ){ lprec = ap10; rprec = ap11; }
    else if( opr_ == op_mixn ){ lprec = ap10; rprec = ap9; }
    else if( opr_ == op_vprod ){ lprec = ap2; rprec = ap3; }
    else if( opr_ == op_plus || opr_ == op_minus ){
      lprec = left_ ? ap0 : ap2; rprec = ap1;   // binary vs. unary
    }
    ...
    /* Invisible multiplication */
      ...
      else if( right_->opr_ == op_deriv ){ lprec = ap6; rprec = ap7; }
      //??? { lprec = ap4; rprec = ap5; }
    ...
    else if( opr_ == op_deriv ){ rprec = ap5; }

    /* Logical operators */
    else if( is_rel( opr_ ) || opr_ == op_is_int ){
      lprec = tp8; rprec = tp9;
    }
    else if( opr_ == op_not ){ rprec = tp9; }
    else if( opr_ == op_and ){ lprec = tp6; rprec = tp7; }
    else if( opr_ == op_or ){ lprec = tp4; rprec = tp5; }
  ...
  /* Setting the value of extra_ */
  inline void set_extra( unsigned uu ){ extra_ = uu; }
...
bool try_fail_leq(
  ...
  for( unsigned ii = var_c_first; ii < var_f_cnt; ++ii ){
    ...
    if(
      xx <= -1 || xx >= 1 ||
      var_used[ ii ].type == vtp_Z || var_used[ ii ].type == vtp_idx
    ){ test_step[ ii ] = 1; }
...
void check_logic( expression *e1, expression *e2, op_type rel_op ){
  ...
  do{
    ...
    // if( gs_u_impl_f ){  //??? The other option seems unnecessary.
    if(
      ( v1 == tv_T && tv_no_T( v2 ) && rel_op != op_limp ) ||
      ( v2 == tv_T && tv_no_T( v1 ) && rel_op != op_impl )
    ){ test_fail( v1, v2 ); return; }
    // }else if(
    //   ( tv_max( v1 ) < tv_min( v2 ) && rel_op != op_impl ) ||
    //   ( tv_min( v1 ) > tv_max( v2 ) && rel_op != op_limp )
    // ){ test_fail( v1, v2 ); return; }
...
expression *arithm_easy( expression *e1, op_type opr, expression *e2 ){
  if( opr == op_plus ){
    ...
    if( e2 == expr_1 ){ return e1; }
    if( e1->opr() == op_e2718 && e2->opr() == op_ln ){ return e2->right(); }
  }
  return 0;
}
...
expression *arithm_easy( op_type opr, expression *e2 ){
  if( opr == op_hpar || opr == op_plus ){ return e2; }
    ...
    if( opr == op_ln || opr == op_log || opr == op_cot ){ return expr_numu; }
    if(
    ...
    if( opr == op_ln || opr == op_log ){ return expr_0; }
  }
  else if( opr == op_ln ){
    if( e2->opr() == op_e2718 ){ return expr_1; }
    if( e2->opr() == op_pow && e2->left()->opr() == op_e2718 ){
      return e2->right();
    }
  }
  return 0;
...
expression *logic_simplify( expression *ee ){
  ...
  // if( opr == op_harr ){
  // if( opr == op_rarr || opr == op_impl ){ return l_or( l_not( e1 ), e2 ); }
  // if( opr == op_limp ){ return l_or( l_not( e2 ), e1 ); }
  if( opr == op_harr /*|| opr == op_leq*/ ){
    return l_or( l_and( e1, e2 ), l_and( l_not( e1 ), l_not( e2 ) ) );
  }
  //??? if( opr == op_limp ){ return new_logic( e2, op_impl, e1 ); }
...
expression *domain( expression *ee ){
  ...
  if( opr == op_div ){ return l_and( dom, l_nq_0( e2 ) ); }
  if( opr == op_mixn ){
  ...
  if( is_sin_like( opr ) ){
    if( opr == op_ln || opr == op_log ){ dom = l_and( dom, l_gt_0( e2 ) ); }
...
inline bool parse_sin_like(){
  return
    parse_is( tkn_ln ) || parse_is( tkn_log ) ||
...
bool is_setting_tkn(){
  ...
  if(
    ...
    tkn_now == tkn_ban_comp || tkn_now == tkn_f_ban_der ||
...
*/
    ...
    if( !ls_b_nodes && tkn_now == tkn_b_nodes ){
      ...
      if( !ls_b_nodes ){
        ...
        pgh_msg( "Bonus is given, if the complexity of the final expression"
          " is at most " );
    ...
    if( tkn_now == tkn_f_ban ){
      if( gs_verbose ){
        pgh_msg( "The final expression must not contain" );
      }
      get_token();
      while( !parse_is( tkn_semic ) && tkn_now != tkn_eoi ){
        op_type opr = op_tkn( tkn_now );
        if( gs_verbose ){ out_print( ' ' ); out_op( opr ); }
    ...
    if( tkn_now == tkn_ok_text ){
      ...
      while( inp_chr ){
        if( inp_chr == '\n' ){
          inp_get_chr();
          if( inp_chr == '\n' ){ break; }
          if( not_first ){ gs_ok_text.push_back( ' ' ); }
        }
        not_first = true;
        if( inp_chr == '&' ){ gs_ok_text += "&amp;"; }
        else if( inp_chr == '<' ){ gs_ok_text += "&lt;"; }
        else if( inp_chr == '>' ){ gs_ok_text += "&gt;"; }
        else if( inp_chr == '\"' ){ gs_ok_text += "&quot;"; }
        else{
          if( inp_chr == '#' ){ inp_get_chr(); }
          gs_ok_text.push_back( inp_chr );
        }
        inp_get_chr();
      }
      get_token(); matched = true;
...
expression *parse_arithm_atom( pm_type pm_now ){
  ...
  if( !( pm_now & pm_int ) && parse_is( tkn_root ) ){
    ...
    if( parse_is( tkn_number ) ){
      ...
      get_token();
    }else if( !( pm_now & pm_mod ) && parse_is_var( vtp_R, pm_now ) ){
...
expression *parse_fact( pm_type pm_now ){
  ...
  while( tkn_now == tkn_excl ){ get_token(); ee = new_expr( op_fact, ee ); }
...
expression *parse_simple_index( pm_type pm_now, bool & allow_const ){
  ...
  if(
    ( !( pm_now & pm_mod ) && parse_is_var( vtp_Z, pm_now ) ) ||
    ( pm_now & pm_mod && parse_is_var( vtp_mod, pm_now ) )
  ){
    unsigned vv = to_var_idx( tkn_val );
...
expression *parse_not( pm_type pm_now ){
  if( parse_is( tkn_not, tkn_excl, tkn_Unot ) ){
    get_token();
    expression *ee = new_expr( op_not, parse_not( pm_now | pm_was_not ) );
    if( par_expr ){ parse_error(); }
    return ee;
  }
...
expression *parse_and( pm_type pm_now ){
  ...
  while(
    ...
    get_token(); ee = new_expr( ee, op_and, parse_not( pm_now ) );
    if( par_expr ){ parse_error(); }
  }
  return ee;
}
...
expression *parse_or( pm_type pm_now ){
  ...
  while(
    ...
    get_token(); ee = new_expr( ee, op_or, parse_and( pm_now ) );
    if( par_expr ){ parse_error(); }
  }
  return ee;
}
...
expression *parse_impl( pm_type pm_now ){
  ...
  if( par_expr || !parse_is( tkn_rarr, tkn_Urarr ) ){ return ee; }
  get_token(); ee = new_expr( ee, op_rarr, parse_impl( pm_now ) );
  if( par_expr ){ parse_error(); }
  return ee;
}
...
/* Draw parse trees of a chain of expressions. */
...
void parse_logic_chain( pm_type pm_now ){
  now_expr = 0;
  op_type rel_op = op_type( 0 );
  ...
  do{
    ...
    now_expr = parse_predicate( pm_now );
    if(
      !(
    ...
    /* Check the current token. */
    parse_settings();
    if( pb_mode == pb_equation && parse_is( tkn_original ) ){
      now_expr = 0; rel_op = op_type( 0 ); been_up = false;
...
void check_array_claim( expression *e1, expression *e2 ){
  if( err_mode ){ return; }
  ...
  while( true ){
    ...
    for( ; ii < sz; ++ii ){
      ...
      if( jj >= int( arr_max_size ) - 1 ){ var_array[ ii ] = 0; }
...
void parse_array_start(){
  ...
  if( !parse_is_var( vtp_R, 0 ) ){ parse_error(); return; }
  ...
  out_html( ' ' ); out_idx_name( v_arr ); out_am( '[' );
...
void parse_array_chain(){
  ...
  do{
    ...
    if(
      !(
        parse_is( tkn_hArr, tkn_UhArr ) || is_setting_tkn() ||
        parse_is_top_tkn()
      )
    ){ parse_error(); }
    ...
    out_am( op_AM[ op_leq ] ); err_set_warning( check_level );
    if( err_proven <= err_mode && err_mode <= err_plausible ){
      out_html( "</span>" ); err_reset();
    }
    out_html( '\n' );
...
void parse_given_roots(){
  ...
  while( parse_is( tkn_vv, tkn_du, tkn_2vbar, tkn_or, tkn_Uor ) ){
    get_token();
    if( !parse_is_var( vtp_R, pm_no_new_free ) ){
...
int main(){
  ...
  do{
    ...
    if( is_problem_mode( tkn_nwprbl, tkn_Nwprbl, pb_mode, 0 ) ){}
      ...
      0 ) ){}
    else if( is_problem_mode( tkn_parse, tkn_Parse, pb_parse_tree,
      "Parse tree" ) ){}
     ...
     "Propositional logic" ) ){}
      ...
      "Array claim" ) ){ parse_array_start(); }
    if( pb_mode == pb_modulo ){ mod_rel = true; }

    /* Read user-given settings. */
    parse_settings( true );

    /* If no error has been detected, check a solution chain. */
    if( !err_mode ){

      /* Process domain restriction. */
      if(
        (
          pb_mode == pb_arithm || pb_mode == pb_draw_function ||
          pb_mode == pb_equation || pb_mode == pb_modulo ||
          pb_mode == pb_prop_logic
        ) && !inp_ff_exam && parse_is( tkn_assume )
      ){

        /* Parse domain restriction. */
        get_token();
        pm_type pm_now = pm_no_qua;
        //???if( ls_ban_comp || ( inp_ff_exam && !ls_allow_comp ) ){
          //pm_now |= pm_no_comp;
        //}
        if( pb_mode == pb_arithm || pb_mode == pb_draw_function ){
          dom_expr = parse_predicate( pm_now );
        }else if( pb_mode == pb_equation ){
          dom_expr = parse_predicate( pm_now | pm_no_new_free );
        }else if( pb_mode == pb_prop_logic ){
          dom_expr = parse_predicate( pm_tv | pm_now );
        }else if( pb_mode == pb_modulo ){
          dom_expr = parse_predicate( pm_mod | pm_now );
        }
        ...
        if(
          !dom_empty && ( pb_mode == pb_prop_logic || pb_mode == pb_modulo )
        ){
          first_test_combination(); dom_empty = true;
          do{
            truth_val t1 = eval_tv( dom_expr );
            if( tv_may_T( t1 ) ){ dom_empty = false; break; }
            // if( !gs_u_impl_f && tv_may_U( t1 ) )
            // { dom_empty = false; break; }
          }while( next_test_combination() );
        }
    ...
    ls_allow_comp = ls_ban_comp = ls_hide_expr = false;
